...
def create_base_layout(cell, total_fingers, total_fins, mos_type, stack = 1, multiplier = 1, orientation = "V", gate_type="cm", body_contact = True):
    ...
    def create_cell(cell_i, bulk_on):
        ...
        if mos_type == "P":
            ...
            add_region(cell_i, (x_i - 200, y_i - 200), (x_f+200, y_f+400), "NW")
        ...
        if bulk_on:
            ...
            y_s = b_y + bulk_h / 2 - lp['M1']['width'] / 2
    ...
    for i in range(multiplier):
        ...
        if orientation == "V":
            create_cell(cell_i, bulk_on=(i + 1) // multiplier)
            ...
            create_cell(cell_i, bulk_on=True)
            p = cell_i.bounding_box()
